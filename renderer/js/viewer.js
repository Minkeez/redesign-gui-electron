...
function deleteHiddenFiles(find_str, cb) {
  ...
  hiddenDir = fullPath(".dice", "");
  console.log(
    "removing hidden files that include " + find_str + " from " + hiddenDir
  );
  fs.readdir(hiddenDir, (err, dir) => {
    // count up the number of potential files to delete
    var numExistingFiles = 0;
    if (!dir) return;
    for (var i = 0; i < dir.length; i++) {
      if (dir[i].includes(find_str)) numExistingFiles++;
    }
...
function loadImageSequence(cb) {
  ...
  if (
    $("#equalizeHistCheck")[0].checked ||
    ($("#analysisModeSelect").val() == "tracking" && showStereoPane == 1)
  ) {
...
$("#loadSubsetFileInput").change(function (evt) {
  if (
    confirm(
      "Importing a subset locations file will reset all ROIs. Continue loading?"
    )
  ) {
...
function reloadVideoImages(index, loadData = true) {
  ...
  if (videoPathLeft != "undefined" || videoPathRight != "undefined")
    if (
      index < Number($("#startPreviewSpan").text()) ||
      index > Number($("#endPreviewSpan").text())
    ) {
      alert("invalid index");
      return;
    }
  ...
  if (
    ...
    showStereoPane == 1 &&
    ($("#showSegmentationCheck")[0].checked ||
      $("#showTrackingCheck")[0].checked)
  ) {
    // signifies tracklib
    ...
    // otherwise just diplay the raw frame
    ...
    if (videoPathLeft != "undefined")
      updateVideoDisplayImage(videoPathLeft, offsetIndex, "left", function () {
        showDeformedROIs();
      });
    if (videoPathRight != "undefined")
...
function setScrollerText(val) {
  ...
  else $("#currentPreviewSpan").text(val);
}
...
$("#frameScroller")
  ...
  .change(function () {
    ...
    if ($("#fileSelectMode").val() == "list") {
      ...
      if (Number($(this).val()) < 0) {
        ...
        if (defImagePathsLeft.length > $(this).val()) {
          updatePreviewImage({
            argsIn: args,
            srcPath: defImagePathsLeft[index].path,
            dest: "left",
          });
          $("#defImageListLeft li:eq(" + index.toString() + ")").addClass(
            "def-image-ul-selected"
          );
        } else {
          resetPlotlyViewer("left");
        }
        if (defImagePathsRight.length > $(this).val()) {
          updatePreviewImage({
            argsIn: args,
            srcPath: defImagePathsRight[index].path,
            dest: "right",
          });
          $("#defImageListRight li:eq(" + index.toString() + ")").addClass(
            "def-image-ul-selected"
          );
        } else {
          resetPlotlyViewer("right");
    ...
    if (selectHasValue("stepSelect", $(this).val())) {
      ...
      console.log(
        "not updating step select since " +
          $(this).val() +
...
$("#videoGoToIndex").keypress(function (event) {
  if (event.keyCode === 13) {
    if (
      $(this).val() < Number($("#startPreviewSpan").text()) ||
      $(this).val() > Number($("#endPreviewSpan").text())
    ) {
      alert("invalid index");
      return;
    }
...
$("#rightRefInput").change(function (evt) {
  ...
  updatePreviewImage({ srcPath: file.path, dest: "right" }, function () {
...
$("#leftRefInput").change(function (evt) {
  ...
  updatePreviewImage({ srcPath: file.path, dest: "left" }, function () {
...
$("#leftDefInput").change(function (evt) {
  ...
  if (files) {
    ...
    for (var i = 0, l = files.length; i < l; i++) {
      ...
      $("#defImageListLeft").append(
        "<li class='defListLi' id='defListLi_" + i + "'>" + fileName + "</li>"
      );
