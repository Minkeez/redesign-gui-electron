        ...
        var shapes = [
          {
            ...
            x0: 0,
            y0: 0,
            x1: div.layout.images[0].sizex,
            y1: div.layout.images[0].sizey,
            line: { width: 0 },
          },
        ];
        ...
        if (div.data[i].mode.includes("lines")) {
          ...
          if (!isNaN(frame)) {
            ...
            for (j = 0; j < div.data[i].frame.length; ++j) {
              if (div.data[i].frame[j] == frame) inFrame = true;
            }
            if (!inFrame) div.data[i].line.color += "00";
          }
        }
        if (div.data[i].mode.includes("markers")) {
          if (div.data[i].marker) {
            for (j = 0; j < div.data[i].marker.color.length; ++j) {
              if (div.data[i].marker.color[j].length == 9)
                // reset the colors in case they were turned transparent previously
                div.data[i].marker.color[j] = div.data[i].marker.color[
                  j
                ].substring(0, 7); //'#ffffff00' -> '#ffffff';
            }
            if (!isNaN(global_id)) {
              for (j = 0; j < div.data[i].marker.color.length; ++j) {
...
function deletePlotlyTraces(dest, name) {
  ...
  var ids = [];
  for (var i = 0; i < div.data.length; ++i) {
    if (div.data[i].name) if (div.data[i].name.includes(name)) ids.push(i);
  }
  if (ids.length > 0) {
...
function updatePreviewImage(update, cb) {
  ...
  args.push(destPath);
  if (argsIn.length > 0) {
    for (var i = 0; i < argsIn.length; ++i) args.push(argsIn[i]);
  } else args.push("filter:none"); // no filter applied, but non .png images are converted to png
  //    console.log(args);

  var child_process = require("child_process");
  var readline = require("readline");
  var proc = child_process.spawn(execOpenCVServerPath, args, {
    cwd: workingDirectory,
  }); //,maxBuffer:1024*1024});
  proc.on("error", function () {
    alert(
      "DICe OpenCVServer failed: invalid executable: " + execOpenCVServerPath
    );
  });
  proc.on("close", (code) => {
    ...
    if (code >= 0 && code < 4) {
      ...
      fs.stat(destPath, function (err, stat) {
        if (err == null) {
          var layoutUpdate = {};
          if (div.layout.images) {
            layoutUpdate["xaxis.range"] = [
              div.layout.xaxis.range[0],
              div.layout.xaxis.range[1],
            ];
            layoutUpdate["yaxis.range"] = [
              div.layout.yaxis.range[0],
              div.layout.yaxis.range[1],
            ];
          } else {
          ...
          var existingShapes = [];
          if (div.layout)
            if (div.layout.shapes) existingShapes = div.layout.shapes;
          var viewBoxIndex = -1;
          for (var i = 0; i < existingShapes.length; ++i)
            if (existingShapes[i].name)
              if (existingShapes[i].name === "viewBox") viewBoxIndex = i;
          if (viewBoxIndex < 0) {
            existingShapes.push({
              name: "viewBox",
              type: "rect",
          ...
          cb(code);
          if (dest == "left" || dest == "right") checkValidInput();
        }
      });
    } else {
      cb(code);
    ...
    .on("line", function (line) {
      ...
      if (debugConsoleDivId.length > 0) {
        if (debugConsoleDivId.length > 0) {
          ...
          $(debugConsoleDivId).scrollTop(
            $(debugConsoleDivId).get(0).scrollHeight
          );
        }
      } else {
        console.log(line);
      ...
      if (line.includes("BUFFER_OUT") && line.includes("IMAGE_WIDTH")) {
        ...
        if (dest == "left") refImageWidth = imgWidth;
      }
      if (line.includes("BUFFER_OUT") && line.includes("IMAGE_HEIGHT")) {
        ...
        if (dest == "left") refImageHeight = imgHeight;
      }
    });
}
...
function resizePreview() {
  ...
  var pvs = [
    document.getElementById("plotlyViewerLeft"),
...
function getPreviewConfig(dest) {
  ...
  var showSubsetLocationsButton = {
    ...
    icon: dotsIcon,
    click: () => {
      if (!$("#runLi").is(":visible")) return;
      drawSubsetCoordinates();
    },
  };
  let importIcon = {
    width: 24,
  ...
  if ($("#analysisModeSelect").val() == "tracking") {
    if (showStereoPane == 1) {
      // signifies tracklib (stereo tracking)
...
function livePlotDims() {
  var result = { numLivePlotPts: 0, livePlotLineActive: false };
...
function drawSubsetCoordinates() {
  ...
  if (
    ...
    } else if ($("#analysisModeSelect").val() == "subset") {
      // if doing subset analysis and custom coordinates are not defined, preview where the subsets will end up
      if (
        ($("#fileSelectMode").val() == "list" &&
          $("#frameScroller").val() == $("#frameScroller").attr("min")) ||
        ($("#fileSelectMode").val() == "sedquence" &&
          $("#frameScroller").val() == $("#refIndex").val()) ||
        ($("#fileSelectMode").val() == "video" &&
          $("#frameScroller").val() == $("#videoRefIndex").val())
      ) {
...
function getSubsetCoordinatesTrace() {
  ...
  if (result >= 0) return allTraces[result];
  else return {};
}
    ...
    if (plotlyDivLeft.layout.shapes) {
      ...
      for (var i = 0; i < shapes.length; ++i) {
        ...
        if (name) {
          ...
          else if (shapes[i].name == name) returnShapes.push(shapes[i]);
        } else {
          returnShapes.push(shapes[i]);
        }
      }
...
$("#plotlyViewerLeft").on("plotly_relayout", function () {
  ...
  if (updateLivePlotLine() || assignShapeNames() || checkForInternalShapes()) {
  ...
  // the subset coordinates trace may not exist, if not and the analsis mode is tracking add one
    ...
    centroids.x &&
    $("#analysisModeSelect").val() == "tracking" &&
    showStereoPane == 0
  ) {
    var scatterTrace = {
...
$("#plotlyViewerLeft").mousemove(function (event) {
  ...
  var xInDataCoord = parseInt(
    coordsXaxisLeft.p2c(event.offsetX - coordsLeftLeft)
  );
  var yInDataCoord = parseInt(
    coordsYaxisLeft.p2c(event.offsetY - coordsTopLeft)
  );
  document.getElementById("leftPos").innerText =
    "[" + xInDataCoord + "," + yInDataCoord + "]";
});
...
$("#plotlyViewerRight").mousemove(function (event) {
  ...
  //    // TODO if outside image width/height return without updating
  var xInDataCoord = parseInt(
    coordsXaxisRight.p2c(event.offsetX - coordsLeftRight)
  );
  var yInDataCoord = parseInt(
    coordsYaxisRight.p2c(event.offsetY - coordsTopRight)
  );
  document.getElementById("rightPos").innerText =
    "[" + xInDataCoord + "," + yInDataCoord + "]";
});

$("#plotlyViewerLeft").on("click", function (event) {
  // note: not plotly_click, to register clicks anywhere in the DOM, not just on a plotly plot
...
function computeEpipolarY0Y1(event, dest) {
  ...
  if (dest == "right") {
    xInDataCoord = parseInt(
      coordsXaxisRight.p2c(event.offsetX - coordsLeftRight)
    );
    yInDataCoord = parseInt(
      coordsYaxisRight.p2c(event.offsetY - coordsTopRight)
    );
  }
  ...
  args.push(calPath);
  if (dest == "left") args.push(1); // indicates left image
  else args.push(2);
  var proc = child_process.spawn(execEpilinePath, args, {
    cwd: workingDirectory,
  }); //,maxBuffer:1024*1024});
  proc.on("error", function () {
    alert(
      "DICe Epiline utility failed: invalid executable: " + execEpilinePath
    );
  });
  proc.on("close", (code) => {
...
if (document.getElementById("plotlyViewerLeft") != null) {
  document
    .getElementById("plotlyViewerLeft")
...
if (document.getElementById("plotlyViewerRight") != null) {
  document
    .getElementById("plotlyViewerRight")
    .addEventListener("mousedown", (ev) => {
      if (calPath == undefined) return;
      if (calPath == "") return;
      if (ev.which == 3) {
...
function angle2d(x1, y1, x2, y2) {
  ...
  while (dtheta < -1.0 * Math.PI) dtheta += 2.0 * Math.PI;
  return dtheta;
}
...
function drawNeighCircle(dest, index) {
  ...
  if (pvl.shapes) existingShapes = pvl.shapes;

  for (var i = 0; i < existingShapes.length; ++i) {
    if (existingShapes[i].name) {
      if (existingShapes[i].name == "neighCircle") existingCircleIndex = i;
...
function deleteShape(index) {
  ...
  if (document.getElementById("plotlyViewerLeft").layout)
    if (document.getElementById("plotlyViewerLeft").layout.shapes)
      if (
        document.getElementById("plotlyViewerLeft").layout.shapes.length > index
      )
        document
          .getElementById("plotlyViewerLeft")
...
function toggleLivePlotVisibility(visibility) {
  ...
  var shapesIndex = -1;
  for (var i = 0; i < shapes.length; ++i) {
    if (shapes[i].name) if (shapes[i].name === "livePlotLine") shapesIndex = i;
  }
  if (shapesIndex >= 0) {
...
function updateLivePlotLine(deleteOnly = false) {
  ...
  for (var i = 0; i < shapes.length; ++i) {
    if (shapes[i].type === "line") {
      if (shapes[i].name) {
        // if it has a name it was a previousl drawn live plot line
        //                if(oldLineIndex>=0)
        //                    alert('error in update live plot line, too many existing lines found');
        if (shapes[i].name === "livePlotLine") oldLineIndex = i;
      } else {
...
function assignShapeNames() {
  ...
  while (i--) {
    if (
      shapes[i].name === undefined &&
      (shapes[i].type === "path" || shapes[i].type === "rect")
    ) {
...
function checkForInternalShapes() {
  ...
  if (
    ...
    // signifies the shape was not inside another one, so check if ROI or obstructed
    ShowDialogBox(
      "Shape type",
      "Select shape type",
      "ROI",
...
function drawBestFitLine(ox, oy, px, py) {
  if (!$("#analysisModeSelect").val() == "subset" || showStereoPane != 1)
    return;
  ...
  for (var i = 0; i < existingShapes.length; ++i) {
    if (existingShapes[i].name)
      if (existingShapes[i].name === "bestFitLine") bestFitLineIndex = i;
  }
...
function updateNeighInfoTrace(dest, index) {
  ...
  if (pvStereo.data) {
    // remove the stereo neigh scatter trace if it exists
    ...
    if (index < 0) {
      // means turn off the neigh info trace
    ...
    var stereoScatterId = pv.data.findIndex((obj) => {
      // remove any stereo neigh scatters from clicks in the othe image
      return obj.name === "tracklibStereoNeighScatter";
    });
    if (stereoScatterId >= 0) stereoScatterIds.push(stereoScatterId);
    if (index < 0) {
      stereoScatterId = pv.data.findIndex((obj) => {
        // remove any stereo neigh scatters from clicks in the othe image
        return obj.name === "tracklibNeighScatter";
      });
      if (stereoScatterId >= 0) stereoScatterIds.push(stereoScatterId);
  ...
  for (var i = 0; i < ids.length; ++i) {
    ...
    text.push(
      "(" +
        ...
        ", " +
        parseFloat(
          (diffAreas[i] * 100.0) / (areas[i] - diffAreas[i])
        ).toPrecision(3) +
        "%)" +
        "<br>" +
        "gray: " +
        ...
        diffGrays[i] +
        ", " +
        parseFloat((diffGrays[i] * 100.0) / 255.0).toPrecision(3) +
        "%)" +
        "<br>" +
        "dist: " +
        parseFloat(dists[i]).toPrecision(2) +
        " (Δ:" +
        parseFloat(diffDists[i]).toPrecision(2) +
        ", " +
        parseFloat(
          (diffDists[i] * 100.0) / (dists[i] - diffDists[i])
        ).toPrecision(3) +
        "%)" +
        "<br>" +
        "Δangle: " +
        parseFloat(angles[i]).toPrecision(3) +
        "<br>" +
        "disparity: " +
        parseFloat(disparities[i]).toPrecision(3) +
        "<br>" +
        "fail: " +
        failCode
    );
  ...
  var stereoDiffAreas =
    pv.data[scatterTraceId].stereoNeighInfo[index].diffAreas;
  var stereoEpiDists =
    pv.data[scatterTraceId].stereoNeighInfo[index].distsFromEpi;
  var stereoDisparities =
    pv.data[scatterTraceId].stereoNeighInfo[index].disparities;
  var stereoFailCodes =
    pv.data[scatterTraceId].stereoNeighInfo[index].failCodes;
  ...
  for (var i = 0; i < stereoIds.length; ++i) {
    ...
    stereoText.push(
      "(" +
        stereoPx[stereoIds[i]] +
        "," +
        stereoPy[stereoIds[i]] +
        ") <br>" +
        "stereo id: " +
        pvStereo.data[scatterTraceIdStereo].stereoGlobalId[stereoIds[i]] +
        "<br>" +
        "frame: " +
        myFrame +
        "<br>" +
        ...
        ", " +
        parseFloat(
          (stereoDiffAreas[i] * 100.0) / (stereoAreas[i] - stereoDiffAreas[i])
        ).toPrecision(3) +
        "%)" +
        "<br>" +
        "dist from epi: " +
        parseFloat(stereoEpiDists[i]).toPrecision(2) +
        "<br>" +
        "stereo disparity: " +
        parseFloat(stereoDisparities[i]).toPrecision(3) +
        "<br>" +
        "fail: " +
        failCode
    );
...
function drawEpipolarLine(dest, y0, y1) {
  ...
  for (var i = 0; i < existingShapes.length; ++i) {
    if (existingShapes[i].name) {
      if (existingShapes[i].name === "epipolarLine") existingEpipolarIndex = i;
    }
  }
  if (existingEpipolarIndex >= 0) {
    // assume if one exists all three exist
